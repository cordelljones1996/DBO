...
getSymbols("DBO",
           from = "2007-01-01",
           to = "2020-07-21")
chartSeries(DBO["2020-04"])
chartSeries(DBO)

# Daily Returns
return <- CalculateReturns(DBO$DBO.Close)
return <- return[-1]
hist(return)
...
chart.RollingPerformance(R = return["2007::2020"],
                         width = 52,
                         FUN = "sd.annualized",
                         scale = 252,
                         main = "Invesco DB Oil Fund (DBO) Monthly Rolling Volatility")
...
f2019 <- ugarchforecast(data = return["/2019-01"],
                        fitORspec = sfinal,
                        n.ahead = 252)
f2020 <- ugarchforecast(data = return["/2020-04"],
                        fitORspec = sfinal,
                        n.ahead = 252)
par(mfrow = c(1,1))
plot(sigma(f2019))
...
plot.zoo(fitted(sim))
plot.zoo(sigma(sim))
p <- 7.27*apply(fitted(sim), 2, 'cumsum') + 7.27
matplot(p, type = "l", lwd = 3,
        main = "Invesco DB Oil Fund (DBO) Predicted Value",
        xlab = "Trading Days",
        ylab= "USD")
